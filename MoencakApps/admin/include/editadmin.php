<?php
  if(isset($_GET['data'])){
    ...
    //get admin
    $sql = "SELECT `a`.`id_admin`, `a`.`nama`, `a`.`email`, `a`.`username`, `a`.`password`, `a`.`level`, `a`.`foto` FROM `admin` `a` where`id_admin`='$id_admin'";
          ...
          <section class="content">
            <div class="card card-info">
              ...
              <div class="card-footer">
                <div class="col-sm-12">
                  <button type="submit" class="btn bg-primer float-right text-white"><i class="fas fa-save"></i> Simpan</button>
                </div>  
              </div>
              <!-- /.card-footer -->
            </form>

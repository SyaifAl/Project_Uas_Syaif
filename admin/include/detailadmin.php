...
// include('../koneksi/koneksi.php');
  if(isset($_GET['data'])){
    $id_admin = $_GET['data'];
    //get admin
    $sql = "SELECT `a`.`id_admin`, `a`.`nama`, `a`.`email`, `a`.`username`, `a`.`password`, `a`.`level`, `a`.`foto`, `j`.`nama_jalur` FROM `admin` `a` JOIN jalur `j` on `a`.`id_jalur` = `j`.`id_jalur` WHERE `id_admin` = '$id_admin' GROUP BY `id_admin`";

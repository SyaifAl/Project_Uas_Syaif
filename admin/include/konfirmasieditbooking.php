...
if(isset($_SESSION['id_booking'])){
    ...
    if(empty($tgl_pendakian)){
	    header("Location:index.php?include=edit-booking&data=$id_booking&notif=editkosong&jenis=tgl_pendakian");
	   ...
	   $sql = "UPDATE `booking` set `tgl_pendakian`='$tgl_pendakian',`tgl_turun`='$tgl_turun', `jumlah_pendaki`='$jumlah_pendaki', `id_jalur`='$id_jalur',`tgl_transaksi`='$tgl_transaksi',`bukti_transaksi`='$nama_file', `status_pembayaran`='$status_pembayaran', `harga_simaksi`='$harga_simaksi' WHERE `id_booking`='$id_booking'";
	mysqli_query($koneksi,$sql);
    }else{
        $sql = "UPDATE `booking` set `tgl_pendakian`='$tgl_pendakian',`tgl_turun`='$tgl_turun', `jumlah_pendaki`='$jumlah_pendaki', `id_jalur`='$id_jalur', `tgl_transaksi`='$tgl_transaksi',`status_pembayaran`='$status_pembayaran', `harga_simaksi`='$harga_simaksi' WHERE `id_booking`='$id_booking'";
	mysqli_query($koneksi,$sql);
    }

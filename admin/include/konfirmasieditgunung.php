...
if(isset($_SESSION['id_gunung'])){
    ...
    $lokasig_file = $_FILES['foto_gunung']['tmp_name'];
    $gunung_file = $_FILES['foto_gunung']['name'];
    $direktorig = 'foto_gunung/'.$gunung_file;
    ...
    $sql_f = "SELECT `foto_gunung` FROM `gunung` WHERE `id_gunung`='$id_gunung'";
    ...
    if (empty($nama_gunung)) {
        header("Location:index.php?include=edit-gunung&data=$id_gunung&notif=editkosong&jenis=nama_gunung");
	    ...
	    if(move_uploaded_file($lokasi_file,$direktori) && move_uploaded_file($lokasig_file,$direktorig)){
            if(!empty($sop) && !empty($foto_gunung)){
                unlink("sop/$sop");
                unlink("foto_gunung/$foto_gunung");
            }
            $sql = "UPDATE `gunung` set `nama_gunung`='$nama_gunung', `desc_singkat`='$desc_singkat', `desc`='$desc', `letak_geografis`='$letak_geografis',`ketinggian`='$ketinggian', `sop`='$nama_file', `harga_simaksi`='$harga_simaksi', `id_provinsi`='$id_provinsi', `foto_gunung` = '$gunung_file'  WHERE `id_gunung`='$id_gunung'";
        ...
        //         $sql = "UPDATE `gunung` set `foto_gunung`='$gunung_file',`nama_gunung`='$nama_gunung', `desc_singkat`='$desc_singkat', `desc`='$desc', `letak_geografis`='$letak_geografis',`ketinggian`='$ketinggian', `harga_simaksi`='$harga_simaksi', `id_provinsi`='$id_provinsi' WHERE `id_gunung`='$id_gunung'";
        //         mysqli_query($koneksi,$sql);
        // }
        else{
